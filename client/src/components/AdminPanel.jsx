...
function AdminPanel({ onBack }) {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post('http://localhost:4000/api/admin/login', login);
  ...
  const handleCreateSession = async (e) => {
    ...
    try {
      await axios.post('http://localhost:4000/api/session', {
  ...
  const deleteSession = async () => {
    if(window.confirm('Supprimer cette session ?')) {
      try {
        await axios.delete(`http://localhost:4000/api/session/${sessionName}`, { headers: { Authorization: `Bearer ${token.trim()}` } });
  ...
  const fetchSessions = async (jwtToken) => {
    try {
      const res = await axios.get('http://localhost:4000/api/sessions', { headers: { Authorization: `Bearer ${jwtToken.trim()}` } });
                  ...
                  <tr key={name} style={{ borderBottom: '1px solid #e2e8f0' }}>
                    ...
                    <td style={{ padding: 8 }}>
                      <button style={{ background: '#e53e3e', color: 'white', border: 'none', borderRadius: 6, padding: '4px 12px', fontWeight: 500, cursor: 'pointer' }} onClick={async () => {
                        if(window.confirm('Supprimer cette session ?')) {
                          try {
                            await axios.delete(`http://localhost:4000/api/session/${name}`, { headers: { Authorization: `Bearer ${token.trim()}` } });
